use crate::{mock::*, Error, Event, GeneralizedDestroyWitness, Proposal, ReceivedAssets, ReceivedCollections, ReceivedCols, ReceivedStruct, SentAssets, SentStruct};
use frame_support::assert_noop;
use pallet_nfts::{CollectionConfigFor, CollectionSettings, MintSettings};
use sp_runtime::{AccountId32, BoundedVec};
use pallet_nfts::Event::Destroyed;
...
fn try_sending_collection_that_doesnt_exist() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            XcNFT::collection_x_transfer(RuntimeOrigin::signed(ALICE), COLLECTION_ID, Some(COLLECTION_ID), 2000.into(), None),
...
fn try_sending_collection_that_user_doesnt_own() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            XcNFT::collection_x_transfer(RuntimeOrigin::signed(BOB), 0, Some(COLLECTION_ID), 2000.into(), None),
...
fn try_voting_on_proposal_expired(){
    ...
    new_test_ext().execute_with(|| {
        ...
        let _ = XcNFT::collection_x_transfer_vote(RuntimeOrigin::signed(ALICE), 1, crate::Vote::Aye);

        System::assert_last_event(RuntimeEvent::XcNFT(Event::ProposalExpired {
            proposal_id: 1,
        }));
    });
}
...
fn vote_on_proposal_successfuly() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_last_event(RuntimeEvent::XcNFT(Event::CrossChainPropoposalVoteRegistered { proposal_id: 1, voter: ALICE, vote: crate::Vote::Aye }));
    });
}
...
fn try_claiming_nft_success() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::XcNFT(Event::NFTClaimed { collection_claimed_from: 1, asset_removed: 0, collection_claimed_to: 0, asset_claimed: 0 }));
...
fn try_collection_parse_empty_successful() {
    new_test_ext().execute_with(|| {
        ...
        let _ = XcNFT::parse_collection_empty(RuntimeOrigin::signed(ALICE), 1, None, BoundedVec::new(), None);

        System::assert_has_event(RuntimeEvent::XcNFT(Event::CollectionReceived { origin_collection_id: 1, received_collection_id: 1, to_address: ALICE }));
    });
}
...
fn try_parse_collection_burn_successful(){
    new_test_ext().execute_with(|| {
        ...
        let destroy_witness = GeneralizedDestroyWitness {
            item_meta: 0,
            item_configs: 0,
            attributes: 0,
        };
        ...
        System::assert_has_event(RuntimeEvent::NFTs(pallet_nfts::Event::Destroyed { collection: 0 }));
    });
}
...
fn try_parse_collection_metadata_successful() {
    new_test_ext().execute_with(|| {
        ...
        let _ = XcNFT::parse_collection_metadata(RuntimeOrigin::signed(ALICE), 0, BoundedVec::new());

        System::assert_has_event(RuntimeEvent::NFTs(pallet_nfts::Event::CollectionMetadataSet { collection: 0, data: BoundedVec::new() }));
    });
}
...
fn try_parse_collection_owner_successful() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::NFTs(pallet_nfts::Event::OwnerChanged { collection: 0, new_owner: BOB }));
    });
}
...
fn try_parse_nft_burn_successful() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::NFTs(pallet_nfts::Event::Burned { collection: 0, item: 0, owner: ALICE }));
    });
}
...
fn try_parse_nft_metadata_successful() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::NFTs(pallet_nfts::Event::ItemMetadataSet { collection: 0, item: 0, data: BoundedVec::new() }));
    });
}
...
fn try_parse_nft_owner_successful() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::NFTs(pallet_nfts::Event::Transferred { collection: 0, item: 0, from: ALICE, to: BOB }));
    });
}
...
fn try_parse_nft_transfer_no_collection() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(XcNFT::parse_nft_transfer(RuntimeOrigin::signed(ALICE),0,0,BoundedVec::new(),0,0,1000.into()), Error::<Test>::CollectionDoesNotExist);
    });
}
...
fn try_parse_nft_transfer_already_received() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(XcNFT::parse_nft_transfer(RuntimeOrigin::signed(ALICE),0,0,BoundedVec::new(),0,0,1000.into()), Error::<Test>::NFTAlreadyReceived);
    });
}
...
fn try_parse_nft_transfer_not_collection_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(XcNFT::parse_nft_transfer(RuntimeOrigin::signed(BOB),0,0,BoundedVec::new(),0,0,1000.into()), Error::<Test>::NotCollectionOwner);
    });
}
...
fn try_parse_nft_transfer_not_existing_nft() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(XcNFT::parse_nft_transfer(RuntimeOrigin::signed(ALICE),0,0,BoundedVec::new(),0,0,1000.into()), Error::<Test>::NFTExists);
    });
}
...
fn try_parse_nft_transfer_successful(){
    ...
    let _ = NFTs::create(RuntimeOrigin::signed(ALICE), ALICE, def_config);
    let _ = XcNFT::parse_nft_transfer(RuntimeOrigin::signed(ALICE),0,0,BoundedVec::new(),0,0,1000.into());
    System::assert_has_event(RuntimeEvent::XcNFT(Event::NFTReceived { origin_collection_id: 0, origin_asset_id: 0, received_collection_id: 0, received_asset_id: 0, to_address: ALICE}));
});
}
...
fn try_parse_nft_transfer_return_to_origin(){
    new_test_ext().execute_with(|| {
        ...
        let _ = XcNFT::parse_nft_transfer(RuntimeOrigin::signed(ALICE),0,0,BoundedVec::new(),0,0,ParachainInfo::parachain_id());
        System::assert_has_event(RuntimeEvent::XcNFT(Event::NFTReturnedToOrigin { returned_from_collection_id: 0, returned_from_asset_id: 0, to_address: ALICE }));
...
fn parse_collection_same_owner_successful(){
    new_test_ext().execute_with(|| {
        ...
        let _ = XcNFT::parse_collection_same_owner(RuntimeOrigin::signed(ALICE), Some(def_config), BoundedVec::new(), nfts.clone(), 1000.into(), 0, None);
        System::assert_has_event(RuntimeEvent::XcNFT(Event::CollectionWithNftsReceived { collection_id: 0, items: nfts.clone() }));
    });
}
...
fn parse_collection_diff_nft_owners_successful(){
    new_test_ext().execute_with(|| {
        ...
        let _ = XcNFT::parse_collection_diff_owners(RuntimeOrigin::signed(ALICE), Some(def_config), BoundedVec::new(), nfts.clone(), 1000.into(), 0, None);
        System::assert_has_event(RuntimeEvent::XcNFT(Event::CollectionWithNftsDiffOwnersReceived { collection_id: 0, items: nfts.clone() }));
    });
}